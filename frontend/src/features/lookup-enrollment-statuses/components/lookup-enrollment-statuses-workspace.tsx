...
export function LookupEnrollmentStatusesWorkspace() {
  ...
  const [editingLookupEnrollmentStatusId, setEditingLookupEnrollmentStatusId] = React.useState<number | null>(
    null,
  );
  const [formState, setFormState] = React.useState<LookupEnrollmentStatusFormState>(DEFAULT_FORM_STATE);
  ...
  React.useEffect(() => {
    ...
    const stillExists = lookupEnrollmentStatuses.some((item) => item.id === editingLookupEnrollmentStatusId);
    if (!stillExists) {
  ...
  const handleSubmitForm = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing && editingLookupEnrollmentStatusId !== null) {
      if (!canUpdate) {
        setFormError("لا تملك صلاحية lookup-enrollment-statuses.update.");
        return;
      }
    ...
    if (!canCreate) {
      setFormError("لا تملك صلاحية lookup-enrollment-statuses.create.");
      return;
    }
      ...
      <Card className="h-fit border-border/70 bg-card/80 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {isEditing ? "تعديل حالة قيد" : "إنشاء حالة قيد"}
          </CardTitle>
          <CardDescription>
            {isEditing ? "تحديث حالة القيد." : "إضافة حالة قيد جديدة."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!canCreate && !isEditing ? (
            <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
              لا تملك صلاحية <code>lookup-enrollment-statuses.create</code>.
            </div>
          ) : (
            <form className="space-y-3" onSubmit={handleSubmitForm} data-testid="enrollment-form">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="ACTIVE"
                  required
                  data-testid="enrollment-form-code"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="منتظم"
                  required
                  data-testid="enrollment-form-name-ar"
                />
              </div>
              ...
              <label className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                ...
                <input
                  ...
                  data-testid="enrollment-form-active"
                />
              </label>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  data-testid="enrollment-form-submit"
                >
                  {isFormSubmitting ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/70 pt-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!pagination || pagination.page <= 1 || lookupEnrollmentStatusesQuery.isFetching}
              >
                السابق
              </Button>
              <Button
