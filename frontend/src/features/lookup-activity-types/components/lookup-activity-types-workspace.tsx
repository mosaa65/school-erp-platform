...
export function LookupActivityTypesWorkspace() {
  ...
  const [editingLookupActivityTypeId, setEditingLookupActivityTypeId] = React.useState<number | null>(
    null,
  );
  const [formState, setFormState] = React.useState<LookupActivityTypeFormState>(DEFAULT_FORM_STATE);
  ...
  React.useEffect(() => {
    ...
    const stillExists = lookupActivityTypes.some((item) => item.id === editingLookupActivityTypeId);
    if (!stillExists) {
  ...
  const handleSubmitForm = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing && editingLookupActivityTypeId !== null) {
      if (!canUpdate) {
        setFormError("لا تملك صلاحية lookup-activity-types.update.");
        return;
      }
    ...
    if (!canCreate) {
      setFormError("لا تملك صلاحية lookup-activity-types.create.");
      return;
    }
      ...
      <Card className="h-fit border-border/70 bg-card/80 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {isEditing ? "تعديل نوع نشاط" : "إنشاء نوع نشاط"}
          </CardTitle>
          <CardDescription>
            {isEditing ? "تحديث نوع النشاط." : "إضافة نوع نشاط جديد."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!canCreate && !isEditing ? (
            <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
              لا تملك صلاحية <code>lookup-activity-types.create</code>.
            </div>
          ) : (
            <form className="space-y-3" onSubmit={handleSubmitForm} data-testid="activity-form">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="SPORTS"
                  required
                  data-testid="activity-form-code"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="رياضي"
                  required
                  data-testid="activity-form-name-ar"
                />
              </div>
              ...
              <label className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                ...
                <input
                  ...
                  data-testid="activity-form-active"
                />
              </label>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/70 pt-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!pagination || pagination.page <= 1 || lookupActivityTypesQuery.isFetching}
              >
                السابق
              </Button>
              <Button
