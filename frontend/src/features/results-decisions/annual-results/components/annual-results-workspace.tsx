        ...
        <div key={row.id} className="rounded-md border border-border/60 p-2 text-xs">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="font-medium">
              {formatNameCodeLabel(row.subject.name, row.subject.code)}
            </span>
          ...
          <div className="mt-1 text-muted-foreground">
            {formatAnnualGradeTerms(row)} | الإجمالي: {row.annualTotal} | %:{" "}
            {row.annualPercentage ?? 0}
          </div>
        </div>
      ))}
    </div>
              ...
              <div className="rounded-md border border-dashed p-2 text-xs text-muted-foreground">
                {calcTermsQuery.isPending
                  ...
                  : calcTermsQuery.data && calcTermsQuery.data.length > 0
                    ? `الفصول المعتمدة: ${calcTermsQuery.data
                        .map((term) => `${term.name} (${term.code})`)
                        .join(" | ")}`
