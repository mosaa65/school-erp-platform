...
export function LookupAbilityLevelsWorkspace() {
  ...
  const [editingLookupAbilityLevelId, setEditingLookupAbilityLevelId] = React.useState<number | null>(
    null,
  );
  const [formState, setFormState] = React.useState<LookupAbilityLevelFormState>(DEFAULT_FORM_STATE);
  ...
  React.useEffect(() => {
    ...
    const stillExists = lookupAbilityLevels.some((item) => item.id === editingLookupAbilityLevelId);
    if (!stillExists) {
  ...
  const handleSubmitForm = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing && editingLookupAbilityLevelId !== null) {
      if (!canUpdate) {
        setFormError("لا تملك صلاحية lookup-ability-levels.update.");
        return;
      }
    ...
    if (!canCreate) {
      setFormError("لا تملك صلاحية lookup-ability-levels.create.");
      return;
    }
      ...
      <Card className="h-fit border-border/70 bg-card/80 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {isEditing ? "تعديل مستوى قدرة" : "إنشاء مستوى قدرة"}
          </CardTitle>
          <CardDescription>
            {isEditing ? "تحديث مستوى القدرة." : "إضافة مستوى قدرة جديد."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!canCreate && !isEditing ? (
            <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
              لا تملك صلاحية <code>lookup-ability-levels.create</code>.
            </div>
          ) : (
            <form className="space-y-3" onSubmit={handleSubmitForm} data-testid="ability-form">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="ممتاز"
                  required
                  data-testid="ability-form-name-ar"
                />
              </div>
              ...
              <label className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                ...
                <input
                  ...
                  data-testid="ability-form-active"
                />
              </label>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  data-testid="ability-form-submit"
                >
                  {isFormSubmitting ? (
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-border/70 pt-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!pagination || pagination.page <= 1 || lookupAbilityLevelsQuery.isFetching}
              >
                السابق
              </Button>
              <Button
